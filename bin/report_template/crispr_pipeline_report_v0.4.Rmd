...
```{r contrast-scatters-save-child, eval=!is.RSL, echo=FALSE, include=FALSE, error=TRUE}
...
<!-- 
This outputs the scatter plots in the main document; instead they are now otuput as separate files
OBS! Error is displayed if no suitable groups were found, i.e. the values "group" in comparisons.txt are all unique


```{r eval=!is.RSL, echo=FALSE, results='asis'}
options(knitr.duplicate.label = "allow")
scatter_plots <- vector(mode = "list", length = length(pairs_to_plot))
