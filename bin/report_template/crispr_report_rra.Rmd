...
```{asis, echo=!is.RSL, eval=gene_id_status)}
...
`r if(gene_id_status){"##### Negative Selection {.tabset}"}`




```{r include=FALSE, eval=gene_id_status}
#negative selection
...
`r if(gene_id_status){"###### Gene Ontology"}`
...
```{r, eval=gene_id_status, child='crispr_report_gsea_prep.Rmd'}
```
...
`r if(gene_id_status){"##### Positive Selection {.tabset}"}`
