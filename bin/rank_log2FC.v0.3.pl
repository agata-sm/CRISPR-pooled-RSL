...
# version 0.3
# change syntax of string contatanation to be compatible with perl 5.30.x
...
my $script_name="rank_log2FC.v0.3.pl";
...
for (@sorted){
	...
	#print Dumper \@sorted_lvl1;

	print "OOOOps $number_of_hashes\n";
	...
	foreach my $i (1..$number_of_hashes){
		...
		#my $sgRNA_score="$sgRNA_2prnt::$score_2prnt"; this used to work on older perl versions (pre 5.30)
...
my $header="sgRNA\tGene\tlogFC\trank_poslFC\trank_neglFC";
...
foreach my $sgRNA_score (sort keys %ranks_per_sgRNA) {
	...
	my $gene=$sgRNA_gene{$sgRNA};


	print OUTFILE "$sgRNA\t$gene\t$score\t$rank_poslFC\t$rank_neglFC\n";
