...
for (i in my.cutoffs){
  ...
  raw_freq_r_log=log10(raw_freq_r[-1,])

  plot3=ggplot(data=raw_freq_r_log, aes(x=log10(readcount), y=frequency, colour=sample, fill=sample)) + geom_histogram(bins = 100) +
  facet_wrap(~sample)+
    scale_fill_viridis(discrete=TRUE,option="turbo")+
    scale_colour_viridis(discrete=TRUE,option="turbo")+
    theme_bw() + theme(plot.margin = unit(c(0.5, -0.5, 0, -0.5), "cm"))+ 
...
Reads per RSL-guide plotted with the reduced range on the X and Y axes displaying the lower-left quandrant of the plot (i.e. region where most of the signal can be observed, as most RSL-sgRNA combinations do not tend to gather high numbers of reads).
...
Reads per RSL-guide plotted with the reduced range on the Y axis and X axis on log10 scale.
...
grid.arrange(grobs = hist.reads_per_RSLsgRNA2.log, ncol=2, top = "Reads per RSL-sgRNA combination. Reduced axes range.", heights=unit(plotheights,"cm"))
